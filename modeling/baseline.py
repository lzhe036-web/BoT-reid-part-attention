...
class MultiGranularityDynamicGate(nn.Module):
    ...
    (G2-local-only) feeds only ``[z2, z4, z6]``.  The ``concat_z4_z6``
    ablation feeds ``[z4, z6]`` and produces exactly two normalized weights.
    Its z2 descriptor is still computed by the part head, but deliberately
    excluded from both the controller and the gated fusion.  No mode adds a
    fourth global-feature gate.
    """

    VALID_GATING_INPUTS = (
        'global', 'concat_global_local', 'concat_local', 'concat_z4_z6'
    )
    ...
    def __init__(self, in_planes, num_scales, temperature=1.0,
        ...
        self.active_scale_indices = (
            (1, 2) if gating_input == 'concat_z4_z6'
            else tuple(range(self.num_scales))
        )
        if gating_input == 'concat_z4_z6' and self.num_scales < 3:
            raise ValueError(
                'concat_z4_z6 gating requires z2, z4, and z6 features'
            )
        ...
        if self.gating_input in (
                'concat_global_local', 'concat_local', 'concat_z4_z6'):
    ...
    def controller_input(self, global_feat, scale_features=None):
        ...
        if self.gating_input == 'concat_z4_z6':
            return torch.cat(
    ...
    def forward(self, global_feat, scale_features=None):
        ...
        weights = (
            probabilities if self.gating_input == 'concat_z4_z6'
