...
正式论文表中的证据 ID 为 `EV-TRAIN-MKT-C2-L03`，等级 E2，独立运行数
`n=1`，训练 seed 为 `not_recorded`。

2026-07-22 新取得的 C2-L01/L05/L10、S2 和完整 CAAT 原始日志/checkpoint
进一步补齐了 λ 与消融证据链，但没有产生高于 C2-L03 的结果，也不改变最终方法
选择。
...
| seed | `not_recorded` | 原始 config、log 未记录固定 seed |
...
- “多次实验均稳定达到 95.0%/87.8%”；
- “结果具有统计显著性”；
- “C2 一定拉近所有数据集、所有 checkpoint 或所有跨摄像头样本距离”。

Market person-only v2 已在特征提取和 pair 生成前排除 `pid<=0`。当前
epoch-120 对照中，跨摄像头同身份距离 mean/median 分别下降 12.670%/13.881%，
mean/median separation gap 分别扩大 12.602%/13.487%。这只能写成当前检索
特征空间中的描述性机制证据；旧 pid=0 污染结果已废弃。
...
补充说明：当前工作区后来新增了显式 `SEED=42` 与确定性采样相关修改，但这些是
历史训练完成后的未提交工作区修改，不能反向证明 C2-L03 当时使用了 seed 42。
