...
ALPHA = 0.1
SCALES = (2, 4, 6)
ANALYSIS_MANIFEST = "g2_e_static_dynamic_alpha0p1_tau0p5_analysis_manifest.json"
...
def _configuration(path):
    ...
    if not math.isclose(float(configuration.MODEL.MULTI_GRANULARITY_STATIC_DYNAMIC_ALPHA),
        ...
        raise ValueError("G2-E analysis requires fixed alpha=0.1")
...
def analyze(config_path, checkpoint_path, output_dir, epoch_stats_path,
    ...
    if render_plots:
        ...
        _plot(rows, "c", "G2-E final local coefficient (c=1+0.1w)", "Density",
              coefficient_base)
    inherited.update({
        ...
        "formula": "concat(g,(1+0.1*w2)z2,(1+0.1*w4)z4,(1+0.1*w6)z6)",
        "coefficient_semantics": (
            "p is probability; w=3p is the dynamic residual branch weight; "
            "residual=0.1*w; c=1+0.1*w is not a probability"
        ),
    })
