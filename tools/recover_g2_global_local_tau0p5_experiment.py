#!/usr/bin/env python
"""Recover/register the formal G2 gate-temperature τg=0.5 ablation.

This executable deliberately reuses the G2 recovery validator after supplying
the candidate's immutable identity.  The base module keeps its original
τg=1.0 defaults, so invoking that tool for the historical G2 run is unchanged.
"""
...
EXPERIMENT_ID = "C2-L03-MGDG-G2-GL-T0P5-S42"
...
RESULT_FILENAME = "g2_tau0p5_formal_result.json"
DEFAULT_CONFIG = (
    REPO_ROOT / "configs" /
...
def _configure_candidate_profile():
    """Set process-local profile constants before calling the shared validator."""
    ...
    g2_recovery.METHOD_VARIANT = (
        "g2_global_local_tau0p5_per_sample_dynamic_gating"
    )
    g2_recovery.METHOD_LABEL = (
        "C2-L03 + G2 Dynamic Gating [g,z2,z4,z6] -> [w2,w4,w6] (τg=0.5)"
    )
    g2_recovery.BASELINE_LABEL = "C2-L03 + G2 Dynamic Gating (τg=1.0)"
...
def main(argv=None):
    ...
    print(json.dumps({
        "created": created, "run_dir": str(run_dir),
