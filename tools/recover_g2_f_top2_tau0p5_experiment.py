#!/usr/bin/env python
"""Recover/register a completed G2-F hard Top-2 formal run without retraining."""
...
EXPERIMENT_ID = "C2-L03-MGDG-G2-F-TOP2-T0P5-S42"
...
DEFAULT_CONFIG = REPO_ROOT / "configs" / "softmax_triplet_c2_l03_multi_granularity_dynamic_gating_g2_f_top2_tau0p5_autodl.yml"
...
def _configure_profile():
    ...
    recovery.EXPECTED_GATING_TAU = .5
    recovery.RESULT_FILENAME = RESULT_FILENAME
    recovery.FORMAL_RESULT_ARTIFACT_TYPE = "g2_f_top2_tau0p5_formal_result"
    recovery.METHOD_VARIANT = "g2_f_hard_top2_sparse_dynamic_gating_tau0p5"
    recovery.METHOD_LABEL = "C2-L03 + G2-F hard Top-2 Dynamic Gating [g,z2,z4,z6]"
    recovery.BASELINE_LABEL = "C2-L03 + G2-A Dynamic Gating (tau_g=0.5)"
    recovery.REQUIRE_RESULT_GATING_TEMPERATURE = True
    recovery.REQUIRE_GATING_TOPK = True
    recovery.EXPECTED_GATING_SPARSIFICATION = "topk"
    recovery.EXPECTED_GATING_TOPK = 2
    recovery.EXPECTED_GATING_TIE_BREAK = "scale_order"
...
def main(argv=None):
    ...
    print(json.dumps({"created": created, "run_dir": str(directory), "run_id": row["run_id"],
