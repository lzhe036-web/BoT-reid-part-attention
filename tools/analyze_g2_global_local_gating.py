...
def _block_rows(state, configuration, checkpoint_sha256,
    ...
    elif profile.gating_input == "concat_z4_z6":
        expected = len(profile.active_scales) * local_dim
        if controller.size(1) != expected:
            raise ValueError(
                "G2-without-z2 controller input width must be {}, got {}".format(
                    expected, controller.size(1)
                )
            )
        boundaries = tuple(
...
def _plot_sample_weight_distribution(series, output_path, pdf_path=None,
    ...
    axis.boxplot(
        series, labels=["w{}".format(scale) for scale in profile.active_scales],
        showmeans=True,
    )
...
def analyze(config_path, checkpoint_path, output_dir, epoch_stats_path,
    ...
    weight_rows, series = _sample_weight_rows(
        samples_path, scales=profile.active_scales
    )
    ...
    manifest = {
        ...
        "controller_output_semantics": (
            "two softmax-normalized weights applied to z4 and z6; z2 is excluded"
            if profile.gating_input == "concat_z4_z6" else
...
def main_for_profile(profile, argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Export observation-only checkpoint-bound gate evidence for {}."
