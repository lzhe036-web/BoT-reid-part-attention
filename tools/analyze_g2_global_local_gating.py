...
def _block_rows(state, configuration, checkpoint_sha256,
    ...
    elif profile.gating_input == "concat_z2_z6":
        expected = len(profile.active_scales) * local_dim
        if controller.size(1) != expected:
            raise ValueError(
                "G2-without-z4 controller input width must be {}, got {}".format(
                    expected, controller.size(1)
                )
            )
        boundaries = tuple(
...
def analyze(config_path, checkpoint_path, output_dir, epoch_stats_path,
    ...
    manifest = {
        ...
        "controller_output_semantics": (
            "two softmax-normalized weights applied to z2 and z6; z4 is excluded"
            if profile.gating_input == "concat_z2_z6" else
