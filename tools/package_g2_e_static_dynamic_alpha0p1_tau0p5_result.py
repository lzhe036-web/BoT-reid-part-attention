...
PACKAGE_NAME = "g2_e_static_dynamic_alpha0p1_tau0p5_result_package"
RESULT_NAME = "g2_e_static_dynamic_alpha0p1_tau0p5_formal_result.json"
...
def _plot_distribution(rows, output_base):
    ...
    for scale in SCALES:
        ...
        axes[1].hist([float(row["c{}".format(scale)]) for row in rows],
                     ...
                     label="c{}=1+0.1w{}".format(scale, scale))
    axes[0].set(title="G2-E residual-branch weights", xlabel="w=3p", ylabel="sample count",
                xlim=(0, 3))
    axes[1].set(title="G2-E final local coefficients", xlabel="c=1+0.1w",
                ylabel="sample count")
    for axis in axes:
        axis.grid(alpha=.25)
...
def package(output_dir, package_dir=None, records_root=None, experiments_path=None):
    ...
    if (result.get("fusion_mode") != "static_concat_plus_gated_residual"
            or float(result.get("static_dynamic_alpha", -1)) != 0.1
            or float(result.get("gating_temperature", -1)) != 0.5):
        raise ValueError("Formal result is not G2-E alpha=.1 tau_g=.5")
    ...
    for row in raw:
        ...
        if (not all(math.isfinite(value) and value >= 0.0
                ...
                or any(not math.isclose(coefficient, 1.0 + 0.1 * weight,
                                         rel_tol=1e-6, abs_tol=1e-8)
                       for coefficient, weight in zip(coefficients, weights))
                or not math.isclose(float(row["alpha"]), 0.1,
    ...
    for source, target in ((source_config, package_dir / "config_source.yml"),
        ...
        shutil.copyfile(str(source), str(target))
    for source in (
            REPO_ROOT / "tools" / "package_g2_e_static_dynamic_alpha0p1_tau0p5_result.py",
            REPO_ROOT / "tools" / "analyze_g2_e_static_dynamic_alpha0p1_tau0p5.py",
            REPO_ROOT / "scripts" / "export_g2_e_static_dynamic_alpha0p1_tau0p5_result_autodl.sh",
    ):
    ...
    for row in raw:
        ...
        enriched = dict(row)
        enriched.update({"version": "G2-E-alpha0p1-tau0p5", "split": split,
    ...
    for script in ("finish_g2_e_alpha_experiment.py", "recover_g2_e_static_dynamic_alpha0p1_tau0p5_experiment.py"):
    ...
    _write(package_dir / "retrieval_metrics.csv",
           ...
           [{"experiment": "G2-E-alpha0p1-tau0p5", "dataset": "market1501",
    ...
    run_manifest = {
        "experiment": "G2-E-alpha0p1-tau0p5", "baseline_branch": "codex/g2-e-static-dynamic-alpha0p3-tau0p5",
        ...
        "fusion_mode": result["fusion_mode"], "static_dynamic_alpha": .1,
    ...
    _atomic_text(package_dir / "README.md", """# G2-E-alpha0p1-tau0p5 formal evidence

This package was generated from one selected formal Market1501 checkpoint. It inherits G2-E alpha=0.3 (`tau_g=0.5`) and changes only alpha in the existing fusion: static `concat(g,z2,z4,z6)` plus residual `0.1 * concat(0,w2*z2,w4*z4,w6*z6)`. Thus the final local coefficients are `c_k=1+0.1*w_k`; `p` is a probability, `w=3p` is the residual-branch weight, and `c` is not a probability.
