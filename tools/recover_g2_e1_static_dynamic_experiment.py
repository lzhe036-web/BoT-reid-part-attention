#!/usr/bin/env python
"""Recover/register a completed E1 α=0.5 formal run without retraining."""
...
EXPERIMENT_ID = "C2-L03-MGDG-G2-E1-STATIC-DYNAMIC-A05-T1-S42"
...
def _configure_profile():
    ...
    recovery.EXPECTED_GATING_TAU = 1.0
    recovery.RESULT_FILENAME = RESULT_FILENAME
    recovery.FORMAL_RESULT_ARTIFACT_TYPE = "g2_e1_static_dynamic_alpha0p5_formal_result"
    recovery.METHOD_VARIANT = "g2_e1_static_concat_plus_gated_residual_alpha0p5"
    recovery.METHOD_LABEL = (
        "C2-L03 + G2 E1 static concat + gated residual "
        "[g,(1+0.5w2)z2,(1+0.5w4)z4,(1+0.5w6)z6]"
    )
    recovery.BASELINE_LABEL = "C2-L03 + original G2 Dynamic Gating (tau_g=1.0)"
...
def main(argv=None):
    ...
    print(json.dumps({"created": created, "run_dir": str(run_dir), "run_id": row["run_id"],
