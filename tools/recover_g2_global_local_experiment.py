...
def _validate_result(output_dir, result, commit, validation_records,
    ...
    if profile.active_scales == (2, 6):
        forbidden = (
            "p4_mean", "p4_std", "p4_min", "p4_max",
            "applied_w4_mean", "applied_w4_std", "dominant_k4_ratio",
        )
        for row in gate_records:
            if row.get("gating_scales") != [2, 6]:
                raise G2RecoveryError("G2-without-z4 gate scales must be [2, 6]")
            if any(field in row for field in forbidden):
                raise G2RecoveryError("G2-without-z4 gate evidence must not record z4")
