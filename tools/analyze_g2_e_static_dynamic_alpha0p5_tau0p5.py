...
ALPHA = 0.5
SCALES = (2, 4, 6)
ANALYSIS_MANIFEST = "g2_e_static_dynamic_alpha0p5_tau0p5_analysis_manifest.json"
...
def _configuration(path):
    ...
    if not math.isclose(float(configuration.MODEL.MULTI_GRANULARITY_STATIC_DYNAMIC_ALPHA),
        ...
        raise ValueError("G2-E analysis requires fixed alpha=0.5")
...
def analyze(config_path, checkpoint_path, output_dir, epoch_stats_path,
            sample_limit=256, device=None, render_plots=False, selected_epoch=None):
    configuration = _configuration(config_path)
    from utils.g2_e_checkpoint_identity import validate
    identity = validate(checkpoint_path, configuration)
    if selected_epoch is None:
        selected_epoch = int(identity["epoch"])
    if int(selected_epoch) != int(identity["epoch"]):
        raise ValueError("Requested analysis epoch differs from checkpoint metadata")
    inherited_manifest_path = g2_analysis.analyze(
        ...
        render_plots=render_plots, selected_epoch=selected_epoch,
    )
    ...
    if render_plots:
        ...
        _plot(rows, "c", "G2-E final local coefficient (c=1+0.5w)", "Density",
              coefficient_base)
    inherited.update({
        ...
        "formula": "concat(g,(1+0.5*w2)z2,(1+0.5*w4)z4,(1+0.5*w6)z6)",
        "coefficient_semantics": (
            "p is probability; w=3p is the dynamic residual branch weight; "
            "residual=0.5*w; c=1+0.5*w is not a probability"
        ),
    })
    ...
    inherited['plots_deferred'] = not render_plots
...
def main(argv=None):
    ...
    print(analyze(args.config_file, args.weight, args.output_dir, args.epoch_stats,
                  args.sample_limit, args.device, args.render_plots))
    return 0
