#!/usr/bin/env python
"""Fail closed unless G2-F differs from G2-A only by declared hard Top-2 gating."""
...
TOP2 = {
    ("MODEL", "MULTI_GRANULARITY_GATING_SPARSIFICATION"): "topk",
    ("MODEL", "MULTI_GRANULARITY_GATING_TOPK"): 2,
    ("MODEL", "MULTI_GRANULARITY_GATING_TIE_BREAK"): "scale_order",
}
ALLOWED_DIFFERENCES = set(TOP2) | {("OUTPUT_DIR",)}
...
def verify(baseline_path, candidate_path):
    ...
    for path, expected in TOP2.items():
        if _at(candidate, path) != expected:
            raise ProtocolError("G2-F mismatch {}".format(".".join(path)))
    ...
    return {
        ...
        "algorithm_variable": "hard Top-2 gate selection with renormalization; tau_g=0.5",
