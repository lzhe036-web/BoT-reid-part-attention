#!/usr/bin/env python
"""Fail closed unless the E1 YAML is a one-variable fusion ablation of G2."""
...
def verify(baseline_path, candidate_path):
    ...
    if _at(candidate, ("MODEL", "MULTI_GRANULARITY_STATIC_DYNAMIC_RESIDUAL")) is not True:
        raise ProtocolError("E1 residual switch must be enabled")
    if float(_at(candidate, ("MODEL", "MULTI_GRANULARITY_STATIC_DYNAMIC_ALPHA"))) != 0.5:
        raise ProtocolError("E1 residual alpha must be 0.5")
    ...
    return {
        ...
        "algorithm_variable": "static concatenation + gated residual, alpha=0.5; tau_g=1.0",
