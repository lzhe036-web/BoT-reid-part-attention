#!/usr/bin/env python
"""Fail closed unless G2-E is exactly the declared alpha=0.3 coefficient ablation."""
...
def _read(path):
    ...
    resolved = cfg.clone()
    resolved.merge_from_file(str(path))
    return resolved
...
def verify(baseline_path, candidate_path):
    ...
    if float(_at(candidate, ("MODEL", "MULTI_GRANULARITY_STATIC_DYNAMIC_ALPHA"))) != 0.1:
        raise ProtocolError("G2-E residual alpha must be 0.1")
    if _at(baseline, ("MODEL", "MULTI_GRANULARITY_STATIC_DYNAMIC_RESIDUAL")) is not True or float(_at(baseline, ("MODEL", "MULTI_GRANULARITY_STATIC_DYNAMIC_ALPHA"))) != 0.3:
        raise ProtocolError("Direct baseline must be residual alpha=0.3")
    ...
    return {
        ...
        "algorithm_variable": "static concatenation + gated residual, alpha=0.1; tau_g=0.5",
