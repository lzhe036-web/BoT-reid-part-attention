#!/usr/bin/env python
"""Finalize formal G2-F hard Top-2 evidence from one selected checkpoint."""
...
from tools.analyze_g2_f_top2_tau0p5 import ANALYSIS_MANIFEST, analyze
...
TAU = 0.5
RESULT_FILENAME = "g2_f_top2_tau0p5_formal_result.json"
ANALYSIS_DIRNAME = "g2_f_top2_tau0p5_analysis"
...
def _load_configuration(config_path, output_dir):
    ...
    checks = {
        ...
        "MODEL.MULTI_GRANULARITY_GATING_NORMALIZATION": (
            str(configuration.MODEL.MULTI_GRANULARITY_GATING_NORMALIZATION), "scaled_softmax"),
        "MODEL.MULTI_GRANULARITY_GATING_SPARSIFICATION": (
            str(configuration.MODEL.MULTI_GRANULARITY_GATING_SPARSIFICATION), "topk"),
        "MODEL.MULTI_GRANULARITY_GATING_TOPK": (
            int(configuration.MODEL.MULTI_GRANULARITY_GATING_TOPK), 2),
        "MODEL.MULTI_GRANULARITY_GATING_TIE_BREAK": (
            str(configuration.MODEL.MULTI_GRANULARITY_GATING_TIE_BREAK), "scale_order"),
    ...
    for name, (actual, expected) in checks.items():
        if actual != expected:
            raise ValueError("Formal G2-F protocol mismatch {}: {!r} != {!r}".format(
                name, actual, expected
            ))
    if Path(str(configuration.OUTPUT_DIR)).resolve() != output_dir:
        raise ValueError("Formal G2-F output directory does not match fixed YAML")
...
def finalize(config_path, output_dir):
    ...
    if not output_dir.is_dir():
        raise FileNotFoundError("G2-F output directory is absent: {}".format(output_dir))
    _load_configuration(config_path, output_dir)
    if _git("branch", "--show-current") != EXPECTED_BRANCH:
        raise ValueError("G2-F finalization requires branch {}".format(EXPECTED_BRANCH))
    ...
    if tuple(int(row["epoch"]) for row in validation) != EXPECTED_EPOCHS:
        raise ValueError("Formal G2-F requires validation epochs {}".format(EXPECTED_EPOCHS))
    ...
    if tuple(int(row["epoch"]) for row in epoch_rows) != tuple(range(1, 121)):
        raise ValueError("Formal G2-F requires one gate-statistics record per epoch")
    ...
    if len(selected_gate) != 1:
        raise ValueError("Selected G2-F epoch has no unique gate statistics")
    analysis_dir, result_path = output_dir / ANALYSIS_DIRNAME, output_dir / RESULT_FILENAME
    if result_path.exists() or analysis_dir.exists():
        raise FileExistsError("Refusing to overwrite existing G2-F formal evidence")
    ...
    result = {
        "experiment": "G2-F hard Top-2 sparse Dynamic Gating (tau_g=0.5)",
        "branch": EXPECTED_BRANCH, "commit": commit, "g2_a_base_commit": G2_A_BASE_COMMIT,
        "seed": 42, "gating_input": "concat([g, z2, z4, z6])", "gating_input_dim": 2816,
        "gating_temperature": TAU, "gate_normalization": "scaled_softmax",
        "gate_outputs": ["w2", "w4", "w6"], "controller": "Linear(2816,3)",
        "controller_parameter_count": (2816 + 1) * 3,
        "gate_sparsification": "topk", "gate_topk": 2, "gate_tie_break": "scale_order",
        "initial_zero_controller_selection": "K2+K4", "weight_scale": 3.0,
        "fusion_formula": "concat(g,w2*z2,w4*z4,w6*z6)", "descriptor_dim": 2816,
        ...
        "top2_selection_boundary_tie_ratio": analysis["selection_boundary_tie_ratio"],
        "evidence": {"config": str(config_path), "config_sha256": sha256_file(config_path),
                     ...
                     "analysis_manifest_name": ANALYSIS_MANIFEST},
    }
...
def main(argv=None):
    ...
    path, result = finalize(args.config_file, args.output_dir)
    print(json.dumps({"result_path": str(path), "metrics": result["metrics"]},
