#!/usr/bin/env python
"""Recover and register a completed formal G2-C experiment without retraining.
...
def _validate_configuration(config_path, resolved_path, output_dir, reproducibility):
    ...
    checks = (
        ...
        (("MODEL", "MULTI_GRANULARITY_GATING_CONTROLLER"), EXPECTED_CONTROLLER),
        (("MODEL", "MULTI_GRANULARITY_GATING_HIDDEN_DIM"), EXPECTED_HIDDEN_DIM),
        ...
        for label, payload in (("source", source), ("resolved", resolved)):
            ...
            if actual != expected:
                raise G2RecoveryError(
                    "G2-C {} config mismatch {}: {!r} != {!r}".format(
    ...
    for label, payload in (("source", source), ("resolved", resolved)):
        ...
        if configured_output != output_dir:
            raise G2RecoveryError(
                "G2-C {} config OUTPUT_DIR {} does not match {}".format(
...
def _validate_reproducibility(reproducibility):
    ...
    if code.get("dirty") is not False:
        raise G2RecoveryError(
            "Formal G2-C launch did not record a clean working tree: {!r}".format(
                code.get("dirty")
            )
        )
    return commit
...
def _validate_result(output_dir, result, commit, validation_records,
    ...
    if result.get("branch") != EXPECTED_BRANCH or result.get("commit") != commit:
        raise G2RecoveryError("G2-C result branch/commit does not match training evidence")
    if result.get("seed") != 42:
        raise G2RecoveryError("G2-C result does not record Seed=42")
    if result.get("gating_input") != "concat([g, z2, z4, z6])":
        raise G2RecoveryError("G2-C result has the wrong controller input")
    if result.get("gate_outputs") != ["w2", "w4", "w6"]:
        raise G2RecoveryError("G2-C result has the wrong gate-output semantics")
    if not _same_number(result.get("gating_temperature"), EXPECTED_GATING_TAU):
        raise G2RecoveryError("G2-C result has the wrong gate temperature")
    if result.get("controller_architecture") != EXPECTED_CONTROLLER:
        raise G2RecoveryError("G2-C result has the wrong controller architecture")
    if result.get("controller_hidden_dim") != EXPECTED_HIDDEN_DIM:
        raise G2RecoveryError("G2-C result has the wrong controller hidden dimension")
    ...
    if observed_validation_epochs != EXPECTED_EPOCHS:
        raise G2RecoveryError(
            "Formal G2-C validation epochs are {}, expected {}".format(
    ...
    if observed_gate_epochs != EXPECTED_GATE_EPOCHS:
        raise G2RecoveryError("Formal G2-C requires one gate-statistics row per epoch")
...
def _validate_analysis(output_dir, result, checkpoint_sha, config_path,
    ...
    if analysis.get("checkpoint_sha256") != checkpoint_sha:
        raise G2RecoveryError("G2-C analysis is bound to a different checkpoint")
    if analysis.get("config_sha256") != sha256_file(config_path):
        raise G2RecoveryError("G2-C analysis is bound to a different config")
    if analysis.get("epoch_statistics_sha256") != sha256_file(gate_stats_path):
        raise G2RecoveryError("G2-C analysis is bound to different gate statistics")
    if analysis.get("controller_architecture") != EXPECTED_CONTROLLER:
        raise G2RecoveryError("G2-C analysis has the wrong controller architecture")
    if analysis.get("controller_hidden_dim") != EXPECTED_HIDDEN_DIM:
        raise G2RecoveryError("G2-C analysis has the wrong controller hidden dimension")
...
def recover(config_path, output_dir, console_log, records_root, experiments_path,
    ...
    if not output_dir.is_dir():
        raise G2RecoveryError("G2-C output directory is absent: {}".format(output_dir))
    ...
    result_path = _require_file(
        output_dir / "g2_c_hidden256_tau0p5_formal_result.json", "G2-C formal result"
    )
    ...
    result = _read_json(result_path, "G2-C formal result")
    ...
    try:
        ...
        result_snapshot = _copy_atomic(
            result_path, temporary_dir / "g2_c_hidden256_tau0p5_formal_result.json"
        )
        ...
        artifacts = {
            ...
            "g2_c_hidden256_tau0p5_formal_result": _file_evidence(result_snapshot),
        ...
        for artifact_type, path in sorted(analysis_files.items()):
            artifacts["g2_c_analysis_{}".format(artifact_type)] = _file_evidence(
        ...
        manifest = {
            ...
            "experiment_family": "C2-L03-MULTI-GRANULARITY-DYNAMIC-GATING-MLP",
            "evidence_id": run_id,
            "run_id": run_id,
            "run_kind": "formal",
            "status": "success",
            ...
            "method_variant": "g2_c_global_local_mlp_hidden256_tau0p5_dynamic_gating",
            "method": "C2-L03 + G2-C MLP Dynamic Gating [g,z2,z4,z6] -> [w2,w4,w6]",
            "dataset": str(_nested(source_config, "DATASETS", "NAMES")),
            "baseline": "Original G2 global-plus-local Dynamic Gating",
            ...
            "gating_controller": EXPECTED_CONTROLLER,
            "gating_controller_hidden_dim": EXPECTED_HIDDEN_DIM,
            "scale_order": "2,4,6",
            ...
            "notes": (
                "Recovered from existing machine-generated G2-C evidence; numeric values "
