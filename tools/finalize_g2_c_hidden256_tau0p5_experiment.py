#!/usr/bin/env python
"""Select and seal the formal G2-C (MLP, tau=0.5) checkpoint evidence."""
...
def _load_configuration(config_path, output_dir):
    ...
    required = {
        ...
        "MODEL.MULTI_GRANULARITY_GATING_CONTROLLER": (
            str(configuration.MODEL.MULTI_GRANULARITY_GATING_CONTROLLER),
            EXPECTED_CONTROLLER,
        ),
        "MODEL.MULTI_GRANULARITY_GATING_HIDDEN_DIM": (
            int(configuration.MODEL.MULTI_GRANULARITY_GATING_HIDDEN_DIM),
            EXPECTED_HIDDEN_DIM,
        ),
    ...
    for field, (actual, expected) in required.items():
        if actual != expected:
            raise ValueError(
                "Formal G2-C protocol mismatch {}: {!r} != {!r}".format(
                    field, actual, expected
                )
            )
    if Path(str(configuration.OUTPUT_DIR)).resolve() != output_dir:
        raise ValueError("Formal G2-C output directory does not match the fixed YAML")
...
def finalize(config_path, output_dir):
    ...
    if not output_dir.is_dir():
        raise FileNotFoundError("G2-C output directory is absent: {}".format(output_dir))
    configuration = _load_configuration(config_path, output_dir)

    branch = _git("branch", "--show-current")
    if branch != EXPECTED_BRANCH:
        raise ValueError(
                "G2-C finalization requires branch {}, got {}".format(
    ...
    if observed_epochs != EXPECTED_EPOCHS:
        raise ValueError(
                "Formal G2-C requires validation epochs {}, got {}".format(
    ...
    if tuple(int(row["epoch"]) for row in epoch_records) != tuple(range(1, 121)):
        raise ValueError("Formal G2-C requires one gate-statistics record per epoch")
    ...
    if len(selected_gate_rows) != 1:
        raise ValueError("Selected G2-C epoch has no unique gate-statistics record")

    analysis_dir = output_dir / "g2_c_hidden256_tau0p5_analysis"
    if analysis_dir.exists():
        raise FileExistsError(
            "Refusing to overwrite existing G2-C analysis: {}".format(analysis_dir)
        )
    ...
    result = {
        "experiment": "G2-C global-plus-local MLP Dynamic Gating (hidden=256, tau=0.5)",
        "branch": branch,
        "commit": commit,
        "seed": 42,
    ...
    delivery_dir = package(output_dir)
    result["delivery_package"] = {
        "path": str(delivery_dir),
        "sha256sums": str(delivery_dir / "SHA256SUMS.txt"),
        "sha256sums_sha256": sha256_file(delivery_dir / "SHA256SUMS.txt"),
    }
    atomic_write_json(result_path, result)
