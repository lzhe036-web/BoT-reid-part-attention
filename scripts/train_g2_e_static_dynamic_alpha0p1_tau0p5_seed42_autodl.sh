#!/usr/bin/env bash
# Formal G2-E: direct alpha=.3 baseline; only alpha changes to 0.1.
set -euo pipefail

EXPECTED_BRANCH="codex/g2-e-static-dynamic-alpha0p1-tau0p5"
G2_A_BASE_COMMIT="63761021a40693694f037d850066deb2237a5c41"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONFIG="${REPO_ROOT}/configs/softmax_triplet_c2_l03_multi_granularity_dynamic_gating_g2_e_static_dynamic_alpha0p1_tau0p5_autodl.yml"
...
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e_static_dynamic_alpha0p1_tau0p5_seed42_market1501"
CONSOLE_LOG="${OUTPUT_DIR}.console.log"
SMOKE_OUTPUT="${G2_E_SMOKE_OUTPUT_DIR:-/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e_static_dynamic_alpha0p1_tau0p5_seed42_market1501_smoke}"
...
test -f "${SMOKE_OUTPUT}/g2_e_smoke_analysis/g2_e_static_dynamic_alpha0p1_tau0p5_analysis_manifest.json" || {
...
python tools/verify_g2_e_static_dynamic_alpha0p1_tau0p5_protocol.py \
  --baseline-config "${BASE_CONFIG}" --candidate-config "${CONFIG}"

python tools/verify_g2_e_alpha_smoke.py --alpha 0.1 \
