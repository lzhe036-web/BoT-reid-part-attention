...
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_without_z6_tau1_seed42_market1501"
...
[[ -z "$(git status --porcelain=v1 --untracked-files=all)" ]] || { echo "Formal G2-without-z6 training requires a clean worktree." >&2; exit 1; }
...
set +e
python tools/finalize_g2_without_z6_experiment.py --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}" 2>&1 | tee -a "${CONSOLE_LOG}"; FINALIZER_STATUS="${PIPESTATUS[0]}"
set -e
...
python tools/recover_g2_without_z6_experiment.py --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}" --console-log "${CONSOLE_LOG}" --started-at-utc "${RUN_STARTED_UTC}" --ended-at-utc "${RUN_ENDED_UTC}" --runtime-seconds "${RUN_RUNTIME_SECONDS}"
