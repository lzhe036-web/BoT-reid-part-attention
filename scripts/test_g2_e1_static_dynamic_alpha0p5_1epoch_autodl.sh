#!/usr/bin/env bash
# One-epoch real-data pipeline check.  This is not a formal result.
set -euo pipefail
...
OUTPUT_DIR="${E1_SMOKE_OUTPUT_DIR:-/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e1_static_dynamic_alpha0p5_tau1_seed42_market1501_smoke}"

cd "${REPO_ROOT}"
test "$(git branch --show-current)" = "${EXPECTED_BRANCH}"
test -z "$(git status --porcelain=v1 --untracked-files=all)" || {
  echo "E1 smoke requires a clean worktree." >&2; exit 1; }
...
python tools/analyze_g2_e1_static_dynamic_alpha.py \
  --config-file "${CONFIG}" --weight "${CHECKPOINT}" \
  --output-dir "${OUTPUT_DIR}/e1_smoke_analysis" \
  --epoch-stats "${OUTPUT_DIR}/dynamic_gating_epoch_stats.jsonl" \
  --sample-limit 32
printf 'E1 smoke pipeline evidence: %s\n' "${OUTPUT_DIR}/e1_smoke_analysis/e1_static_dynamic_analysis_manifest.json"
