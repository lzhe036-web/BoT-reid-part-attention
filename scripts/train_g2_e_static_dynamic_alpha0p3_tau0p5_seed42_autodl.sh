#!/usr/bin/env bash
# Formal G2-E: G2-A tau=.5 plus static concat and fixed alpha=.3 residual.
set -euo pipefail
...
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e_static_dynamic_alpha0p3_tau0p5_seed42_market1501"
CONSOLE_LOG="${OUTPUT_DIR}.console.log"
SMOKE_OUTPUT="${G2_E_SMOKE_OUTPUT_DIR:-/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e_static_dynamic_alpha0p3_tau0p5_seed42_market1501_smoke}"
...
test -f "${SMOKE_OUTPUT}/g2_e_smoke_analysis/g2_e_static_dynamic_alpha0p3_tau0p5_analysis_manifest.json" || {
...
START_EPOCH="$(date +%s)"; START_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  python tools/finalize_g2_e_static_dynamic_alpha0p3_tau0p5_experiment.py \
    --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}"
  python tools/package_g2_e_static_dynamic_alpha0p3_tau0p5_result.py \
    --output-dir "${OUTPUT_DIR}"
  bash scripts/export_g2_e_static_dynamic_alpha0p3_tau0p5_result_autodl.sh "${OUTPUT_DIR}"
} 2>&1 | tee "${CONSOLE_LOG}"
END_EPOCH="$(date +%s)"; END_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

python tools/recover_g2_e_static_dynamic_alpha0p3_tau0p5_experiment.py \
...
printf 'Formal G2-E evidence and archive are ready under: %s\n' "${OUTPUT_DIR}"
