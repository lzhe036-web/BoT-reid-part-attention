...
OUTPUT_DIR="${G2_E_SMOKE_OUTPUT_DIR:-/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_e_static_dynamic_alpha0p1_tau0p5_seed42_market1501_smoke}"
...
CHECKPOINT="$(find "${OUTPUT_DIR}" -maxdepth 1 -type f -name 'resnet50_checkpoint_*.pt' -print -quit)"
test -n "${CHECKPOINT}" && test -f "${CHECKPOINT}"
python tools/analyze_g2_e_static_dynamic_alpha0p1_tau0p5.py \
...
printf 'G2-E smoke evidence: %s\n' \
  "${OUTPUT_DIR}/g2_e_smoke_analysis/g2_e_static_dynamic_alpha0p1_tau0p5_analysis_manifest.json"
