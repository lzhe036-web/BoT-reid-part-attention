...
EXPECTED_SMOKE_OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2l03_soft_min_alignment_tau0p2_lambda0p05_seed42_market1501_smoke"

CURRENT_BRANCH="$(git symbolic-ref --quiet --short HEAD || true)"
if [[ -z "${CURRENT_BRANCH}" ]] || [[ "${CURRENT_BRANCH}" != "${EXPECTED_BRANCH}" ]]; then
  echo "Refusing lambda=0.05 smoke from branch '${CURRENT_BRANCH:-detached HEAD}'; expected '${EXPECTED_BRANCH}'." >&2
  exit 1
fi
if [[ -n "$(git status --porcelain --untracked-files=all)" ]]; then
  echo "Refusing lambda=0.05 smoke from a dirty Git worktree." >&2
  git status --short >&2
  exit 1
fi
if [[ "$(git rev-parse "${PARENT_BRANCH}")" != "${PARENT_COMMIT}" ]] || \
  ...
  echo "Refusing lambda=0.05 smoke: fixed tau-sweep parent lineage differs." >&2
  exit 1
fi
if [[ "$(git rev-parse "${FEATURE_REFERENCE_BRANCH}")" != "${FEATURE_REFERENCE_COMMIT}" ]] || \
  ...
  echo "Refusing lambda=0.05 smoke: fixed Hard feature reference differs." >&2
  exit 1
fi
...
python tools/run_experiment.py \
  --config "${CONFIG}" \
  --experiment-id C2-L03-SOFTMIN-T0P2-LP0P05-S42-SMOKE \
  ...
  --notes "One-epoch Soft-Min lambda sweep smoke; tau=0.2; lambda_p=0.05; K=6; Seed=42."
