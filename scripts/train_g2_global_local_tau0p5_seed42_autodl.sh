#!/usr/bin/env bash
# Formal τg=0.5 candidate.  It never reuses G2 τg=1.0 output paths.
set -euo pipefail

EXPECTED_BRANCH="codex/g2-global-local-gating-tau0p5"
G2_BASE_COMMIT="5a9a2a33e19f3d9dfc3afaf356dc8b9ee3f0f737"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONFIG="${REPO_ROOT}/configs/softmax_triplet_c2_l03_multi_granularity_dynamic_gating_g2_global_local_tau0p5_autodl.yml"
BASE_CONFIG="${REPO_ROOT}/configs/softmax_triplet_c2_l03_multi_granularity_dynamic_gating_g2_global_local_autodl.yml"
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_global_local_tau0p5_seed42_market1501"
...
if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
  printf 'Formal τg=0.5 training requires a clean worktree.\n' >&2
  exit 1
fi
if ! git merge-base --is-ancestor "${G2_BASE_COMMIT}" "${CURRENT_COMMIT}"; then
  printf 'Candidate commit is not descended from formal G2 base %s.\n' "${G2_BASE_COMMIT}" >&2
  exit 1
fi
...
RUN_STARTED_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  python tools/finalize_g2_global_local_experiment.py \
    ...
    --expected-branch "${EXPECTED_BRANCH}" --expected-gating-tau 0.5 \
    --experiment-label "G2 global-plus-local Dynamic Gating (tau=0.5)" \
    --result-filename g2_tau0p5_formal_result.json \
    --analysis-dirname g2_tau0p5_gating_analysis
} 2>&1 | tee "${CONSOLE_LOG}"
RUN_ENDED_EPOCH="$(date +%s)"
RUN_ENDED_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
RUN_RUNTIME_SECONDS="$((RUN_ENDED_EPOCH - RUN_STARTED_EPOCH))"

python tools/recover_g2_global_local_tau0p5_experiment.py \
  --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}" \
  --console-log "${CONSOLE_LOG}" --started-at-utc "${RUN_STARTED_UTC}" \
  --ended-at-utc "${RUN_ENDED_UTC}" --runtime-seconds "${RUN_RUNTIME_SECONDS}"

printf 'Formal τg=0.5 evidence was registered. Output: %s\n' "${OUTPUT_DIR}"
