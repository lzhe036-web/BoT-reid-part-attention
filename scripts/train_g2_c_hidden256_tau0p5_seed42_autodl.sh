...
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_c_hidden256_tau0p5_seed42_market1501"
...
if [[ -n "$(git status --porcelain=v1 --untracked-files=all)" ]]; then
  printf 'Formal G2-C training requires a clean worktree.\n' >&2
  exit 1
fi
...
RUN_STARTED_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
{
  ...
  python tools/finalize_g2_c_hidden256_tau0p5_experiment.py \
    --config-file "${CONFIG}" \
...
python tools/recover_g2_c_hidden256_tau0p5_experiment.py \
  --config-file "${CONFIG}" \
  --output-dir "${OUTPUT_DIR}" \
