...
EXPECTED_BRANCH="codex/g2-without-z2"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONFIG="${REPO_ROOT}/configs/softmax_triplet_c2_l03_multi_granularity_dynamic_gating_g2_without_z2_autodl.yml"
OUTPUT_DIR="/root/autodl-tmp/experiments/BoT/c2_l03_mgdg_g2_without_z2_tau1_seed42_market1501"
...
[[ -z "$(git status --porcelain=v1 --untracked-files=all)" ]] || { echo "Formal G2-without-z2 training requires a clean worktree." >&2; exit 1; }
...
set +e
python tools/finalize_g2_without_z2_experiment.py --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}" 2>&1 | tee -a "${CONSOLE_LOG}"; FINALIZER_STATUS="${PIPESTATUS[0]}"
set -e
...
python tools/recover_g2_without_z2_experiment.py --config-file "${CONFIG}" --output-dir "${OUTPUT_DIR}" --console-log "${CONSOLE_LOG}" --started-at-utc "${RUN_STARTED_UTC}" --ended-at-utc "${RUN_ENDED_UTC}" --runtime-seconds "${RUN_RUNTIME_SECONDS}"
