...
def _loss_output_to_dict(loss_output):
    ...
    return loss_output, {
        ...
        'loss_same_camera_positive': loss_output.new_tensor(0.0),
...
def create_supervised_trainer(model, optimizer, loss_fn,
    ...
    def _update(engine, batch):
        ...
        return {
            ...
            'loss_same_camera_positive': _item(loss_dict.get('loss_same_camera_positive', 0.0)),
            'same_camera_positive_count': loss_dict.get('same_camera_positive_count', 0),
            'acc': acc.item(),
        }
...
def do_train(
    ...
    RunningAverage(output_transform=lambda x: x['loss_same_camera_positive']).attach(trainer, 'avg_loss_same_camera_positive')
    RunningAverage(output_transform=lambda x: x['same_camera_positive_count']).attach(trainer, 'avg_same_camera_positive_count')
    ...
    def log_training_loss(engine):
        ...
        if ITER % log_period == 0:
            logger.info("Epoch[{}] Iteration[{}/{}] loss_total: {:.3f}, loss_id: {:.3f}, "
                        ...
                        "cross_camera_positive_count: {:.1f}, loss_same_camera_positive: {:.3f}, "
                        "same_camera_positive_count: {:.1f}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(engine.state.epoch, ITER, len(train_loader),
                                ...
                                engine.state.metrics['avg_loss_same_camera_positive'],
                                engine.state.metrics['avg_same_camera_positive_count'],
            ...
            if cfg.MODEL.SAME_CAMERA_POSITIVE_ONLY and engine.state.output['same_camera_positive_count'] == 0:
                logger.info("No same-camera positive anchors in current batch; same-camera auxiliary loss is skipped.")
