...
def gating_scales(configuration):
    """Return the active gate scales without changing part-feature extraction."""
    try:
        value = _config_value(configuration, "MODEL.MULTI_GRANULARITY_GATING_INPUT")
    except (AttributeError, KeyError):
        value = "global"
    return (2, 4) if str(value) == "concat_z2_z4" else (2, 4, 6)
...
class GatingEpochAccumulator(object):
    ...
    def __init__(self, temperature, scales=(2, 4, 6), weight_sum=None):
        ...
        self.weight_sum = float(len(self.scales) if weight_sum is None else weight_sum)
        self.reset()
...
def append_gating_epoch_record(output_dir, epoch, global_iteration,
    ...
    record.update({field: statistics[field] for field in gating_stat_fields(scales)})
...
def validate_dynamic_gating_evidence(
    ...
    for line_number, row in enumerate(rows, 2):
        ...
        for index in range(len(scales)):
            _close(w[index], expected_weight_sum * p[index], "applied gating weight")
