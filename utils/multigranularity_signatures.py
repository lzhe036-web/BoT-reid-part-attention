...
def _is_fusion_only_name(name):
    """Whether a Baseline field belongs solely to an experiment's fusion layer.

    The shared signature intentionally covers feature-map production and local
    descriptor construction, but not a post-descriptor gating/fusion ablation.
    E1 adds a static-concatenation-plus-residual switch after ``z2/z4/z6`` have
    already been built.  Keep that switch out of the *shared* signature while
    retaining it in the separate fusion/gating signature below.
    """
...
def _contains_fusion_only_reference(node):
    """Recognize local names, attributes, and constructor arguments alike."""
